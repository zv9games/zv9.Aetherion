...
impl AetherionEngine {
        ...
        if let Some(signals) = &self.signals_node {
            match message {
                ...
                GenerationMessage::ChunkGenerated(coords) => {
                    ...
                    if let Ok(mut aetherion_signals_instance) = signals.clone().try_cast::<AetherionSignals>() {
                        aetherion_signals_instance.bind_mut().emit_chunk_generated(coords.x as i32, coords.y as i32);
                    }
                }
                GenerationMessage::GenerationComplete => {
                    ...
                    if let Ok(mut aetherion_signals_instance) = signals.clone().try_cast::<AetherionSignals>() {
                        aetherion_signals_instance.bind_mut().emit_build_map_complete();
                    }
                }
    ...
    /// Main engine tick method, called by AetherionOracle.
    #[func]
    pub fn tick(&mut self, current_tick: u64) {
        godot_print!("AetherionEngine: Received tick #{}", current_tick);
    ...
    /// Sets the reference to the AetherionSignals node for emitting core events.
    #[func]
    pub fn set_signals_node(&mut self, signals_node: Gd<Node>) {
        self.signals_node = Some(signals_node);
        info!("AetherionEngine: Signals Node reference set.");
    }
    ...
    pub fn set_tilemap(&mut self, tilemap_node: Gd<TileMap>) {
        ...
        info!("AetherionEngine: TileMap Node reference set.");
    }
        ...
        let result = match conductor_arc.lock() {
            Ok(mut conductor) => {
                ...
                match conductor.start_generation(config) {
                    Ok(_) => {
                        info!("AetherionEngine: Conductor command accepted: START GENERATION.");
                        Ok(())
                    }
                    Err(e) => {
            ...
            Ok(_) => {
                ...
                if let Some(signals) = &self.signals_node {
                    if let Ok(mut aetherion_signals_instance) = signals.clone().try_cast::<AetherionSignals>() {
                        ...
                        aetherion_signals_instance.bind_mut().emit_build_map_start();
                    } else {
                        godot_error!("Signal Emission Failed: Signals Node reference is not of type AetherionSignals.");
                    }
                } else {
                    godot_error!("Signal Emission Failed: AetherionSignals node reference not set.");
                }
            }
            Err(status_msg) => {
    ...
    pub fn shutdown_engine(&mut self) {
        ...
        if let Some(conductor_arc) = self.conductor.take() {
            if let Ok(c) = Arc::try_unwrap(conductor_arc) {
                ...
                godot_error!("AetherionEngine: Cannot fully shutdown Conductor; other references still exist. This is usually okay if other systems hold a shared reference, but it might indicate a leak if unintended.");
            }
        }
    }
}