// aetherion_shared/src/errors.rs
...
pub type AetherionResult<T> = Result<T, AetherionError>;
...
impl From<bincode::Error> for AetherionError {
    fn from(err: bincode::Error) -> Self {
        AetherionError::Serialization(format!("Bincode failure: {}", err))
    }
}
...
impl From<anyhow::Error> for AetherionError {
    fn from(err: anyhow::Error) -> Self {
        AetherionError::External(format!("General anyhow error: {}", err))
    }
}
