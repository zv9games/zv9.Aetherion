...
// --- Modules ---
pub mod model; // This is the change: we make the model module public.
...
pub mod interface {
    pub mod signal;
    pub mod echo_api; // EchoApi is in here
    pub mod bindings;
    pub mod adapter;
    pub mod data_bridge;
}
...
// === CORRECTED CODE ===
// This is your new entry point struct.
// It is empty and its only purpose is to implement ExtensionLibrary.
struct AetherionEngine;

#[gdextension]
unsafe impl ExtensionLibrary for AetherionEngine {
    // The `ExtensionLibrary` macro will automatically find and register
    // all the `GodotClass`es in your crate, including EchoApi.
}
