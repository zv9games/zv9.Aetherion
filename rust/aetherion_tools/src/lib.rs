...
impl AetherionConfig {
    ...
    fn load_from_path(path: &str) -> Result<Self, io::Error> {
        info!("AetherionConfig: Attempting to load configuration from: {}", path);
        ...
        match File::open(path) {
            Ok(mut file) => {
                ...
                // --- SIMULATED PARSING ---
                info!("AetherionConfig: File read successfully. Simulating config override.");
...
pub fn get_config_from_path(path: Option<&str>) -> Result<AetherionConfig, io::Error> {
    ...
    match AetherionConfig::load_from_path(path_to_load) {
        Ok(config) => {
            info!("AetherionConfig loaded from path: {}", path_to_load);
            Ok(config)
        },
        Err(e) => {
            ...
            Ok(AetherionConfig::new_with_defaults())
        }
    }
}
...
static CONFIG: Lazy<AetherionConfig> = Lazy::new(AetherionConfig::new_with_defaults);
